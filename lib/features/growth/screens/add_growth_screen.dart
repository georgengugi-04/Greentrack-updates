...
class _AddGrowthScreenState extends State<AddGrowthScreen> {
          ...
          children: [
              ...
              child: Column(
                children: [
                  Text('${_height.toInt()} cm',
                      style:
                          AppTextStyles.display(32, color: AppColors.purple)),
                  Slider(
                    value: _height,
                    min: 1,
                    max: 200,
            ...
            Row(
              children: [
                IconButton.filled(
                  onPressed: () => setState(
                      () => _leafCount = (_leafCount - 1).clamp(0, 999)),
                ...
                Expanded(
                  child: Center(
                    child: Text('$_leafCount',
                        style:
                            AppTextStyles.display(28, color: AppColors.leaf)),
                  ),
                ),
                IconButton.filled(
            ...
            Row(
              children: [
                Expanded(
                    child: _HealthOption('🟢', 'Healthy', _health,
                        (v) => setState(() => _health = v))),
                const SizedBox(width: 8),
                Expanded(
                    child: _HealthOption('🟡', 'Concern', _health,
                        (v) => setState(() => _health = v))),
                const SizedBox(width: 8),
                Expanded(
                    child: _HealthOption('🔴', 'Attention', _health,
                        (v) => setState(() => _health = v))),
              ],
            ),
            const SizedBox(height: 20),
  ...
  void _submit() {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('📏 Growth record saved for ${_selectedCrop?.name}!')));
    context.pop();
  }
}
